...
class Goal(models.Model):
    ...
    def create(self, user_id, name, iteration_type, frequency, object_time, complete_day=0):
        ...
        user = User()
        p = Goal(user_id=user.read(user_id=user_id),
                 name=name,
    ...
    def read(self, user_id, name):
        p = Goal.objects.get(user_id=user_id, name=name)
        return p

    def update(self, user_id, prev_name, name, iteration_type, frequency, object_time, complete_day):
        # start_day는 default값인 datetime.today() 들어감
        p = Goal.objects.get(user_id=user_id, name=prev_name)
        p.name = name
    ...
    def filter(self, user_id, name):
        p = Goal.objects.filter(user_id=user_id, name=name)
        return p

    def filter(self, user_id):
        p = Goal.objects.filter(user_id=user_id)
        return p

    def is_exist(self, user_id, name):
        exist = True
        try:
            Goal.objects.get(user_id=user_id, name=name)
        except ObjectDoesNotExist:
            exist = False
        return exist
...
class Work(models.Model):
    ...
    def create(self, user_id, name, date, complete_time):
        ...
        w = Work.objects.create(user_id=user.read(user_id=user_id),
                                name=goal.read(user_id=user_id, name=name),
                                date=date,
